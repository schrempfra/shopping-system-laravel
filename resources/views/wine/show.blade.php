<x-app-layout>
    <div class="container mx-auto px-4">
        <div class="wine-details border-b border-gray-300 pb-12 flex flex-col lg:flex-row">
            <div class="flex-none">
                <img src="https://picsum.photos/id/{{ $wine->id }}/200/300" alt="cover">
            </div>
            <div class="lg:ml-12 xl:mr-64">
                <h2 class="font-semibold text-2xl leading-tight mt-1">{{ $wine->title }}</h2>
                ...
                <p class="mt-12">{{ $wine->description }}</p>

                <div class="mt-12" x-data="{ isTrailerModalVisible: false }">
                    <button @click="isTrailerModalVisible = true"
                        class="flex bg-sky-500 text-white font-semibold px-4 py-4 hover:bg-sky-600 rounded transition ease-in-out duration-150">
                        <svg class="w-6" width="24" height="24" viewBox="0 0 24 24" stroke-width="2"
        ...
        <div class="similar-wines-container my-10">
            <h2 class="text-gray-900 uppercase tracking-wide font-semibold">Gleiche Sorte</h2>
            <div class="similar-wines text-sm grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 xl:grid-cols-6 gap-12">
                @foreach ($wine->getSimilarWine() as $wine)
                    <x-wine-card :wine="$wine" />
                @endforeach
            </div> <!-- end similar-wines -->
        </div> <!-- end similar-wines-container -->
    </div>
</x-app-layout>
